...
app.controller('BuscarProveedores', function($scope,$http){
...
app.controller('BuscarProveedorID', function($scope, $http){

	
$scope.look = function(x){
	console.log(x);  //Verificar que se haya ejecutando el click de actualizar
		
	$http.get("http://localhost:8080/FarmaciaJava/api/proveedores/" +x)
	.success(function(response) {
...
app.controller('AgregarProveedor', function($scope, $http){
	...
	$scope.add = function(){
		...
		$http.post("http://localhost:8080/FarmaciaJava/api/proveedores", $scope.data)
		.success(function() {
...
app.controller('ActualizarController', function($scope, $http){
	...
	$scope.actualizar = function(x){
		...
		$http.put("http://localhost:8080/FarmaciaJava/api/proveedores/"+$scope.data.proveedorId, $scope.data)
		.success(function() {
...
app.controller('EliminarProveedor', function($scope, $http){
...
//		Preguntar porque no se ejecutan los mensajes del delete y el .delete porque marca error
		$http['delete']("http://localhost:8080/FarmaciaJava/api/proveedores/"+$scope.data.proveedorId)
		.success(function() {
